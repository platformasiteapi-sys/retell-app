...
const service = ({logger, makeService}) => {
  // Listen on the root path instead of /retell to be less strict
  ...
  svc.on('session:new', async(session) => {
    ...
    try {
      ...
      if (outboundFromRetell) {
        ...
        /* if a carousel of caller IDs is provided, use them in order */
        if (env.callerIdCarousel) {
          const callerIds = env.callerIdCarousel.split(',');
          if (callerIds.length > 0) {
            from = callerIds[carouselIndex].trim();
            carouselIndex = (carouselIndex + 1) % callerIds.length;
          }
        }
        /* Workaround for SIPGATE and maybe others, put User ID as from and CLI in header */
        else if (env.overrideCallerId) {
